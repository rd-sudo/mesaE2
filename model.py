...
class TrafficModel(mesa.Model):
    def __init__(self, width=24, height=24, num_agents=1,
                 left_coords=None, right_coords=None, up_coords=None, down_coords=None, buildings_coords=None, parkings_coords=None):
        super().__init__()
        # Properties for the grid
        buildingLayer = mesa.space.PropertyLayer("building", width, height, np.int64(0), np.int64)
        parkingsLayer = mesa.space.PropertyLayer("parking", width, height, np.int64(0), np.int64)
        ...
        self.grid = mesa.space.MultiGrid(width, height, True, (buildingLayer, parkingsLayer))
...
#VIZUALIZATION
    def set_building_cells(self, buildings_coords, buildingLayer):
        for coord in buildings_coords:
            buildingLayer.set_cell(coord, 1)  # Establecer el valor de la celda a 1 para indicar un edificio
    def set_parking_cells(self, parking_coords, parkingsLayer):
        for coord in parking_coords:
            parkingsLayer.set_cell(coord, 1)  # Establecer el valor de la celda a 1 para indicar un edificio
